...
| 角色 | 分支名 | 状态 |
|------|--------|------|
| 组长 | `dev`（直接在 dev 整合） | ✅ 已创建 |
| 成员一（游戏逻辑） | `feature/game-core` | ✅ 已创建 |
| 成员二（美术设计） | `feature/ui-design` | ✅ 已创建 |
| 成员三（Flask 后端） | `feature/backend-auth` | ✅ 已创建 |
| 成员四（数据库） | `feature/database-score` | ✅ 已创建 |
| 成员五（测试文档） | `feature/test-docs` | ✅ 已创建 |
...
# 成员四（数据库与成绩）
git checkout feature/database-score
...
> 如果提示需要设置上游分支，说明你还没切到正确的分支，确认一下 `git branch` 的输出。
...
## ❌ 注意：不要直接修改 main 或 dev 分支

`main` 分支是最终稳定版本，`dev` 是开发主分支，**都不允许直接推送**。
...
> 🎯 记住：**只在自己的分支上改代码，不碰 main，分支已建好直接用，有问题问组长！**
